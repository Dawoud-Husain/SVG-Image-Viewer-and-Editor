...
clean:
	rm -rf $(BIN)StructListDemo $(BIN)runner $(BIN)xmlExample $(BIN)*.o $(BIN)*.so
...
runner: $(SRC)main.c $(INC)LinkedListAPI.h $(INC)SVG*.h
	$(CC) $(CFLAGS) -I$(XML_PATH) $(SRC)main.c $(SRC)SVG*.c  $(SRC)LinkedListAPI.c -I$(INC) -lxml2 -o $(BIN)runner -lm
